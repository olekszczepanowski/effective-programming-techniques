...
public:
    ...
    explicit CTree(CNode<T>* newRoot){
        root = newRoot;
        isFixed = false;
    }
    CTree(const CTree &&other) noexcept{
        root = other.root;
    ...
    CTree(vector<string> formula){
        isFixed = false;
        root = new CNode<T>(nullptr, formula[0]); // Tworzenie korzenia drzewa na podstawie pierwszego elementu formuły
        ...
        // Przechodzenie przez formułę, zaczynając od indeksu 1, ponieważ pierwszy pointer został użyty do stworzenia korzenia
    ...
    CTree<T>& operator=(CTree<T>&& newTree) noexcept;
...
template <typename T>
CTree<T>& CTree<T>::operator=(CTree<T>&& newTree) noexcept {
    if (this != &newTree) {
        delete root;
        root = newTree.root;
        newTree.root = nullptr;

        variables = std::move(newTree.variables);
    }
    return *this;
}
template <typename T>
CTree<T> CTree<T>::operator+(const CTree<T>& newTree) {
    CTree returnTree(*this);  // Create a copy of the current tree
    ...
    CTree addedElem(newTree);
    parent->getChildren()[0] = std::move(*addedElem.root);
    addedElem.root->setParent(parent);


    for (typename std::map<std::string, std::string>::iterator it = addedElem.variables.begin(); it != addedElem.variables.end(); ++it) {
        returnTree.variables[it->first] = std::move(it->second);
    }

    return returnTree;  // Return the modified tree using move semantics
}
